...
def angle_between(v1, v2):
    ...
    v1_u = unit_vector(v1)
    v2_u = unit_vector(v2)
    return np.arccos(np.clip(np.dot(v1_u, v2_u), -1.0, 1.0))[0]
...
class ComposeSonarLandmarkEvaluator(Evaluable):
    ...
    def __init__(self, transform: Evaluable, landmark: Evaluable):
        super().__init__()
        self._transform: Evaluable = transform # Sonar Pose (coincident with robot pose)
    ...
    def forward(self) -> Node:
        ...
        landmark_robot = transform_child.value.matrix() @ landmark_child.value

        # Get angle between each axis and landmark
        if np.sum(landmark_robot[:3]) > 0:
            distance = np.linalg.norm(landmark_robot[:3])
            alpha = angle_between((1, 0, 0), landmark_robot[:3])
            beta = angle_between((0, 1, 0), landmark_robot[:3])
            gamma = angle_between((0, 0, 1), landmark_robot[:3])
            print(distance, alpha, beta, gamma)
        else:
            distance = 0.0
            alpha = 0.0
            beta = 0.0
